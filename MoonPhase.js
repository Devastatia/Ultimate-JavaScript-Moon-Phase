Date.prototype.getJulianDayNumber = function()
{
  var month = this.getMonth() + 1,
      ...
      y     = this.getFullYear() + 4800 - a,
      m     = month + (12 * a) - 3;
  return day + Math.floor(((153 * m) + 2) / 5) + (365 * y) + Math.floor(y / 4) - Math.floor(y / 100) + Math.floor(y / 400) - 32045;
} // Date.prototype.getJulianDayNumber

Math.getFractionalPart = function(floatingPointNumber)
{
  return (Math.abs(floatingPointNumber) - Math.floor(floatingPointNumber));
} // Math.getFractionalPart
...
function MoonPhase()
{
  ...
  this.getPhase = function()
  {
    ...
    var thisJD = today.getJulianDayNumber(),
        ...
        m0     = 360 * Math.getFractionalPart(k0 * 0.08084821133) + 359.2242 - (0.0000333 * t2) - (0.00000347 * t3),
        m1     = 360 * Math.getFractionalPart(k0 * 0.07171366128) + 306.0253 + (0.0107306 * t2) + (0.00001236 * t3),
        b1     = 360 * Math.getFractionalPart(k0 * 0.08519585128) + 21.2964  - (0.0016528 * t2) - (0.00000239 * t3),
        phase  = 0,
        jday   = 0,
        oldJ,
        f,
    ...
    while (jday < thisJD) {
      f      = f0 + (1.530588 * phase);
      m5     = Math.degreesToRadians(m0 + (phase * 29.10535608));
      m6     = Math.degreesToRadians(m1 + (phase * 385.81691806));
      b6     = Math.degreesToRadians(b1 + (phase * 390.67050646));
    ...
    cached = (thisJD - oldJ) % 30;
    return cached;
  }  // getPhase

  this.render = function(containerId)
  {
    with (document.getElementById(containerId).style) {
      backgroundSize  = "cover";
      backgroundImage = "url(data:image/gif;base64," + moonPhaseImages[this.getPhase()] + ")";
    }
  } // render
